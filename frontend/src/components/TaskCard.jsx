...
const TaskCard = ({ task }) => {
  const { setTaskToEdit, setOpen, setConfirmDelete, setTaskToDelete } = useContext(taskContext);
    ...
    <li className="group bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-700/60 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={() => { setTaskToDelete(task); setConfirmDelete(true); }}
          ...
          <button
            onClick={() => { setTaskToEdit(task); setOpen(true); }}
